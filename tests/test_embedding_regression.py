...
def _generate_patient_package_embeddings(
    ...
    *,
    ...
    """Run patient summarize+embed pipeline from raw note-level input."""
    patient_notes = patient_input[["patient_id", "note_text", "note_date"]].copy()
    patient_summaries = cast(
        pd.DataFrame,
        summarize_patients(
            patient_notes,
            config=config,
